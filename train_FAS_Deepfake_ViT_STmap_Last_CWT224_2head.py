...
from models.ResNet_Xception import ViT_STmap_LastCWT_2heads, ResNet18_STmap_LastCWT_2head


from Load_FAS_ResNet_Xception_STmap_CWTlast_2heads import Spoofing_train, Normaliztion, ToTensor
...
train_FAS_Deepfake_list = '/users/yuzitong/Protocols/FAS_Deepfake_TIFS_Protocols/Joint_FAS_Deepfake/SiW_3DMAD_HKBU_FF_train.txt'
...
def train_test():
    ...
    if finetune==True:
        ...
        model = ResNet18_STmap_LastCWT_2head(pretrained=True)
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        train_data = Spoofing_train(train_FAS_Deepfake_list, root_FAS_Deepfake_dir, transform=transforms.Compose([ ToTensor(),  Normaliztion()]))
        ...
        for i, sample_batched in enumerate(dataloader_train):
            ...
            spoof_label = spoof_label.squeeze(-1)
            
            
            optimizer.zero_grad()
            

            logits1_1, logits1_2, logits2_1, logits2_2, logits1_final, logits2_final =  model(inputs, inputs2) # [B*T, 2]

            
            loss1 =  0.0
            loss2 =  0.0
            loss3 = 0.0
            for kk in range(inputs.shape[0]):
                # DeepFake
                if spoof_label[kk]>1:
                    loss1 =  loss1 + criterion(logits1_2[kk], spoof_label[kk]-2)
                    loss2 =  loss2 + criterion(logits2_2[kk], spoof_label[kk]-2)
                    loss3 =  loss3 + criterion(logits2_final[kk], spoof_label[kk]-2)
                # FAS
                else:
                    loss1 =  loss1 + criterion(logits1_1[kk], spoof_label[kk])
                    loss2 =  loss2 + criterion(logits2_1[kk], spoof_label[kk])
                    loss3 =  loss3 + criterion(logits1_final[kk], spoof_label[kk])
            
            loss1 = loss1/inputs.shape[0]
            loss2 = loss2/inputs.shape[0]
            loss3 = loss3/inputs.shape[0]
            ...
            if i % echo_batches == echo_batches-1:    # print every 50 mini-batches
                ...
                # visualization
                FeatureMap2Heatmap(inputs, inputs2)
        ...
        if epoch % epoch_test == epoch_test-1:    # test every 5 epochs  
            ...
            with torch.no_grad():
                ...
                for i, sample_batched in enumerate(dataloader_test):
                    ...
                    logits1_1, logits1_2, logits2_1, logits2_2, logits, logits2_final  =  model(inputs, inputs2)
                ...
                for i, sample_batched in enumerate(dataloader_test):
                    ...
                    logits1_1, logits1_2, logits2_1, logits2_2, logits1_final, logits  =  model(inputs, inputs2)
                    for test_batch in range(inputs.shape[0]):
                        map_score = 0.0
                        map_score += F.softmax(logits)[test_batch][1]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log', type=str, default="ResNet18_Last_FAS_Deepfake_Joint_STmap_CWT224_2heads", help='log and save model name')
