...
from models.ResNet_Xception import ViT_STmap_LastCWT, ResNet18_STmap_LastCWT


from Load_FAS_ResNet_Xception_STmap_CWTlast import Spoofing_train, Normaliztion, ToTensor
from Load_FAS_ResNet_Xception_STmap_CWTlast_test import Spoofing_valtest, Normaliztion_valtest, ToTensor_valtest
...
def get_inf_iterator(data_loader):
    ...
    while True:
        for i, sample_batched in enumerate(data_loader):
            yield (sample_batched['image_x'], sample_batched['image_x2'], sample_batched['spoofing_label'])
...
def FeatureMap2Heatmap(x, x2):
    ...
    cv2.imwrite(args.log+'/'+args.log + '_x_CWTlast.jpg', org_img)
    ...
    cv2.imwrite(args.log+'/'+args.log + '_x_STmap.jpg', org_img)
...
def train_test():
    ...
    if finetune==True:
        ...
        model = ResNet18_STmap_LastCWT(pretrained=True)
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        train_data_FAS = Spoofing_train(train_FAS_list, root_FAS_Deepfake_dir, transform=transforms.Compose([ToTensor(), Normaliztion()]))
        dataloader_train_FAS = DataLoader(train_data_FAS, batch_size=args.batchsize//2, shuffle=True, num_workers=4)
        
        train_data_DF = Spoofing_train(train_Deepfake_list, root_FAS_Deepfake_dir, transform=transforms.Compose([ToTensor(), Normaliztion()]))
        ...
        for i in range(iternum):
            image_x_FAS, image_x_FAS2, spoofing_label_FAS = next(data1_FAS)
            image_x_DF, image_x_DF2, spoofing_label_DF = next(data1_DF)
            
            image_x_FAS = image_x_FAS.cuda()
            image_x_FAS2 = image_x_FAS2.cuda()
            image_x_DF = image_x_DF.cuda()
            image_x_DF2 = image_x_DF2.cuda()
            spoofing_label_FAS = spoofing_label_FAS.cuda()
            spoofing_label_DF = spoofing_label_DF.cuda()
            ...
            logits1, logits2, logits_fuse =  model(image_x_FAS, image_x_FAS2) # [B*T, 2]

            
            loss1 =  criterion(logits1, spoofing_label_FAS.squeeze(-1))
            loss2 =  criterion(logits2, spoofing_label_FAS.squeeze(-1))
            loss3 =  criterion(logits_fuse, spoofing_label_FAS.squeeze(-1))
            ...
            n = image_x_FAS.size(0)
            loss_absolute.update(loss1.data, n)
            loss_contra.update(loss2.data, n)
            loss_absolute_RGB.update(loss3.data, n)
            ...
            logits1, logits2, logits_fuse =  model(image_x_DF, image_x_DF2) # [B*T, 2]

            
            loss1 =  criterion(logits1, spoofing_label_DF.squeeze(-1))
            loss2 =  criterion(logits2, spoofing_label_DF.squeeze(-1))
            loss3 =  criterion(logits_fuse, spoofing_label_DF.squeeze(-1))
            ...
            if i % echo_batches == echo_batches-1:    # print every 50 mini-batches
                ...
                # visualization
                FeatureMap2Heatmap(image_x_DF, image_x_DF2)
        ...
        if epoch % epoch_test == epoch_test-1:    # test every 5 epochs  
            ...
            with torch.no_grad():
                ...
                for i, sample_batched in enumerate(dataloader_test):
                    ...
                    inputs2 = sample_batched['image_x2'].cuda() 
                    #test_maps = sample_batched['val_map_x'].cuda()   # binary map from PRNet 
        
                    optimizer.zero_grad()
                    
                    logits1, logits2, logits  =  model(inputs, inputs2)
                ...
                FF_test_filename = args.log+'/'+ args.log+'_FF_test.txt' 
                ...
                DFDC_test_filename = args.log+'/'+ args.log+'_DFDC_test.txt' 
                ...
                test_CelebDF_filename = args.log+'/'+ args.log+'_CelebDF_test.txt' 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log', type=str, default="ResNet18_Last_FAS_Deepfake_Joint_STmap_CWT224_Alternating", help='log and save model name')
