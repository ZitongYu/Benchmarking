...
class Spoofing_train(Dataset):
    ...
    def get_single_image_x(self, image_path):
        ...
        image_x = np.zeros((63, 224, 224, 3))
        
        for ii in range(1, 64):
            if ii<10:
                image_jpg = 'CWT_0'+ str(ii) + '.jpg'
            else:
                image_jpg = 'CWT_'+ str(ii) + '.jpg'
            
            image_path_temp = os.path.join(image_path, image_jpg)
            image_x_temp = cv2.imread(image_path_temp)
            image_x_temp = cv2.resize(image_x_temp, (224, 224))
            image_x_temp = seq.augment_image(image_x_temp) 
            
            image_x[ii-1,:,:,:] = image_x_temp
