...
from models.ResNet_Xception import ViT_2heads


from Load_FAS_ResNet_Xception_2heads import Spoofing_train, Normaliztion, ToTensor, RandomHorizontalFlip, Cutout
...
def train_test():
    ...
    if finetune==True:
        ...
        model = ViT_2heads(pretrained=True)
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        train_data = Spoofing_train(train_FAS_Deepfake_list, root_FAS_Deepfake_dir, transform=transforms.Compose([RandomHorizontalFlip(),  ToTensor(), Cutout(), Normaliztion()]))
        ...
        for i, sample_batched in enumerate(dataloader_train):
            ...
            for kk in range(inputs.shape[0]):
                # DeepFake
                if spoof_label[kk]>1:
                    loss_global =  loss_global + criterion(logits2[kk], spoof_label[kk]-2)
                # FAS
                else:
                    loss_global =  loss_global + criterion(logits[kk], spoof_label[kk])
            #loss_global =  criterion(logits, spoof_label.squeeze(-1))
            
            loss_global = loss_global/inputs.shape[0]
            ...
            with torch.no_grad():
                ...
                for i, sample_batched in enumerate(dataloader_test):
                    ...
                    for frame_t in range(inputs.shape[1]):
                        logits, logits2  =  model(inputs[:,frame_t,:,:,:])
                        if frame_t==0:
                ...
                for i, sample_batched in enumerate(dataloader_test):
                    ...
                    for frame_t in range(inputs.shape[1]):
                        logits, logits2  =  model(inputs[:,frame_t,:,:,:])
                        if frame_t==0:
                            logits_accumulate = F.softmax(logits2,-1)
                        else:
                            logits_accumulate += F.softmax(logits2,-1)
                    logits_accumulate = logits_accumulate/inputs.shape[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log', type=str, default="ViT_2heads_FAS_Deepfake_Joint", help='log and save model name')
